...
function assignOrder(c, G, vertNum, sol, numVert)
    ...
    for i = 2:siz
        ...
        if has(G, firstVertex, arr[i]) &&
           sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 1
            add!(order, vertNum[arr[i]], vertNum[firstVertex] + verts)
            add!(order, vertNum[firstVertex] + verts, vertNum[arr[i]] + verts)
        elseif has(G, firstVertex, arr[i]) &&
               sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 0
...
function _compareG(order, i, j, vertNum, sol, arr, verts, firstVertex, G)
    if i == j
        if !has(G, firstVertex, arr[i]) &&
           sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 1 &&
           sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
            add!(order, vertNum[arr[i]], vertNum[arr[i]] + verts)
        elseif !has(G, firstVertex, arr[i]) &&
               sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 1 &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0
    ...
    elseif has(G, arr[i], arr[j]) && sol[(vertNum[arr[j]]-1)*verts+vertNum[arr[i]]] == 1 #if there is an edge and beta is 1
        if (
            has(order, vertNum[arr[j]] + verts, vertNum[firstVertex] + verts) &&
            has(order, vertNum[arr[j]], vertNum[arr[j]] + verts)
        ) #first case
            ...
            elseif has(order, vertNum[arr[i]] + verts, vertNum[firstVertex] + verts) &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0
                add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            end
        elseif (
            has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) &&
            has(order, vertNum[firstVertex] + verts, vertNum[arr[j]])
        ) #sixth case
            add!(order, vertNum[arr[j]], vertNum[arr[i]])
            add!(order, vertNum[arr[i]], vertNum[arr[j]] + verts)
            if has(order, vertNum[firstVertex] + verts, vertNum[arr[i]]) &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
                add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
            elseif has(order, vertNum[firstVertex] + verts, vertNum[arr[i]]) &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0
                add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            end
        elseif (
            has(order, vertNum[arr[j]], vertNum[firstVertex] + verts) &&
            has(order, vertNum[arr[j]] + verts, vertNum[arr[j]])
        ) #second case
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            if has(order, vertNum[arr[i]], vertNum[firstVertex] + verts) &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
                add!(order, vertNum[arr[i]], vertNum[arr[j]] + verts)
            elseif has(order, vertNum[arr[i]], vertNum[firstVertex] + verts) &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0
                add!(order, vertNum[arr[j]], vertNum[arr[i]])
            end
        elseif (
            has(order, vertNum[firstVertex] + verts, vertNum[arr[j]] + verts) &&
            has(order, vertNum[arr[j]] + verts, vertNum[arr[j]])
        ) #fifth case
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            if has(order, vertNum[firstVertex] + verts, vertNum[arr[i]] + verts) &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
                add!(order, vertNum[arr[j]], vertNum[arr[i]] + verts)
            elseif has(order, vertNum[firstVertex] + verts, vertNum[arr[i]] + verts) &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0
                add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]] + verts)
            end
        elseif has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) &&
               has(order, vertNum[firstVertex] + verts, vertNum[arr[j]] + verts) &&
               has(G, arr[j], firstVertex) #case 3
            add!(order, vertNum[arr[j]], vertNum[arr[i]])
            add!(order, vertNum[arr[i]], vertNum[arr[j]] + verts)
            if has(order, vertNum[arr[i]] + verts, vertNum[firstVertex] + verts)
                add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
        ...
        elseif has(order, vertNum[arr[j]] + verts, vertNum[arr[j]]) &&
               has(order, vertNum[firstVertex] + verts, vertNum[arr[j]]) &&
               has(G, arr[j], firstVertex) #case 4
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            if has(order, vertNum[arr[i]], vertNum[firstVertex] + verts)
                add!(order, vertNum[arr[i]], vertNum[arr[j]] + verts)
    ...
    elseif has(G, arr[i], arr[j]) && sol[(vertNum[arr[j]]-1)*verts+vertNum[arr[i]]] == 0 #if there is an edge and beta is 0
        ...
        elseif has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) &&
               has(order, vertNum[firstVertex] + verts, vertNum[arr[j]]) #sixth case
            add!(order, vertNum[arr[j]], vertNum[arr[i]] + verts)
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]] + verts)
            if has(order, vertNum[firstVertex] + verts, vertNum[arr[i]]) &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
        ...
        elseif has(order, vertNum[firstVertex] + verts, vertNum[arr[j]] + verts) &&
               has(order, vertNum[arr[j]] + verts, vertNum[arr[j]]) #fifth case
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]])
            add!(order, vertNum[arr[i]], vertNum[arr[j]])
            if has(order, vertNum[firstVertex] + verts, vertNum[arr[i]] + verts) &&
               sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1
    ...
    elseif !has(G, arr[i], arr[j]) && sol[(vertNum[arr[j]]-1)*verts+vertNum[arr[i]]] == 1 #if there is not an edge and beta is 1
        if has(order, vertNum[arr[j]] + verts, vertNum[arr[j]]) &&
           has(order, vertNum[firstVertex] + verts, vertNum[arr[j]]) &&
           has(G, arr[j], firstVertex) ||
           has(order, vertNum[firstVertex] + verts, vertNum[arr[j]] + verts) &&
           has(order, vertNum[arr[j]] + verts, vertNum[arr[j]]) ||
           has(order, vertNum[arr[j]], vertNum[firstVertex] + verts) &&
           has(order, vertNum[arr[j]] + verts, vertNum[arr[j]]) #case 4, 5 and 2
            add!(order, vertNum[arr[i]], vertNum[arr[j]])
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]])
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
            add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
        elseif has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) &&
               has(order, vertNum[firstVertex] + verts, vertNum[arr[j]] + verts) &&
               has(G, arr[j], firstVertex) #third case
        ...
        elseif has(order, vertNum[arr[j]] + verts, vertNum[firstVertex] + verts) &&
               has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) #case 1
            if has(G, arr[i], firstVertex)
                if sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 1 &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 1
                    add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]])
                elseif sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 1 &&
                       sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 0
                    add!(order, vertNum[arr[i]], vertNum[arr[j]])
                elseif sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 0 &&
                       sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 1
                    add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
                elseif sol[(vertNum[firstVertex]-1)*verts+vertNum[arr[i]]] == 0 &&
                       sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 0
            ...
            elseif sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 1 &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 0
                add!(order, vertNum[arr[i]], vertNum[arr[j]])
                add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]] + verts)
            elseif sol[(vertNum[arr[i]]-1)*verts+vertNum[firstVertex]] == 0 &&
                   sol[(vertNum[arr[i]]-1)*verts+vertNum[arr[j]]] == 1
                add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]])
                add!(order, vertNum[arr[j]] + verts, vertNum[arr[i]])
            end
    ...
    elseif !has(G, arr[i], arr[j]) && sol[(vertNum[arr[j]]-1)*verts+vertNum[arr[i]]] == 0 #if there is not an edge and beta is 0
        if (
           ...
           has(order, vertNum[arr[j]], vertNum[arr[j]] + verts) &&
           has(order, vertNum[firstVertex] + verts, vertNum[arr[j]]) #first case and sixth case
            add!(order, vertNum[arr[j]], vertNum[arr[i]])
            add!(order, vertNum[arr[j]], vertNum[arr[i]] + verts)
            add!(order, vertNum[arr[i]], vertNum[arr[j]] + verts)
            add!(order, vertNum[arr[i]] + verts, vertNum[arr[j]] + verts)
